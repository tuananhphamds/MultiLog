...
class PNHBertCosineLast(BaseModel):
    def __init__(self, cfg, embedding_matrix, log_template_dict, hyper_center):
        super(PNHBertCosineLast, self).__init__(cfg, embedding_matrix, log_template_dict, hyper_center)
    ...
    def _build_model(self):
        ...
        distances = 1 - tf.math.negative(sim_fn(trans_encoded[:, 0], inputs1))
    ...
    def _calculate_scores_all_sessions(self, sess_dct, generator_batch_size):
        ...
        for batch_data in sub_generator.get_batches():
            ...
            for idx, subseq in enumerate(subseqs):
                for i in range(self.cfg['window_size']):
                    ...
                    mask_labels.append([1] + label + [next_labels[idx]])
            ...
            mask_labels = np.reshape(mask_labels, (len(mask_labels), self.cfg['window_size'] + 2))
            ...
            # PROBS
            mask = (mask_labels != 1).reshape(-1, self.cfg['window_size'] + 2, 1)
        ...
        return total_probs, total_nexts, total_scores
        ...
        self.model = load_model(os.path.join(self.cfg['result_path'], model_file),
                                custom_objects={'TransformerEncoder': TransformerEncoder,
                                                              ...
                                                              'acc': custom_metric(acc_fn=acc_fn, vocab_size=self.cfg['vocab_size'],
                                                                                   ignore_index=1),
                                                              'Identity': Identity,
                                                              'contrast_loss': custom_contrast_loss(1)})
    ...
    #                     index = np.where(value == min_value)[0][0]
    #                     filtered_seqs.append(mask_subseqs[idx][index].tolist())
    ...
    #     normal_cls_embedding = self._extract_interested_seqs(normal_sess_dct, generator_batch_size, type='normal', threshold=0.5233)
    #     abnormal_cls_embedding = self._extract_interested_seqs(non_unk_sess_dct, generator_batch_size, type='abnormal', threshold=0.5233)
    ...
    #     with open('cls_embeddings_cosine.pickle', 'wb') as f:
    ...
    def test(self, normal_sess_dct, abnormal_sess_dct, X_val=None, Y_val=None):
        ...
        all_abnormal_probs, all_abnormal_next, all_abnormal_dists = self._calculate_scores_all_sessions(

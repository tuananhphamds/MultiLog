...
class PNHBertEuclideanLast(BaseModel):
    def __init__(self, cfg, embedding_matrix, log_template_dict, hyper_center):
        super(PNHBertEuclideanLast, self).__init__(cfg, embedding_matrix, log_template_dict, hyper_center)
    ...
    def _build_model(self):
        ...
        distances = tf.linalg.norm(trans_encoded[:, 0] - inputs1, axis=1)
    ...
    def _compile(self, num_X_train):
        ...
        self.model.compile(loss={'mask_out': custom_loss(cce=cce, vocab_size=self.cfg['vocab_size'], ignore_index=1),
                                 ...
                                 'contrast_out': custom_contrast_loss(margin=12)},
                           metrics={
    ...
    def _calculate_scores_all_sessions(self, sess_dct, generator_batch_size):
        ...
        for batch_data in sub_generator.get_batches():
            ...
            batch_hyper_center = np.tile(self.hyper_center.reshape(1, 32), (len(mask_subseqs), 1))
